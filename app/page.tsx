...
type E={id:string;sender:string;type:'text'|'image'|'deleted';text:string;imageUrl:string;typing:number;delay:number;storyTime:string;dividerLabel:string};
type S={title:string;contactName:string;subtitle:string;events:E[]};
export default function Home(){
  const[story,setStory]=useState<S|null>(null);
  const[form,setForm]=useState({genre:'Thriller',length:'4–5 minutes',tone:'Creepy',format:'Dating-app chat',premise:'',imageCount:4,pacing:{typing:1800,normal:3200,tension:5500,jump:5000,reveal:8500}});
  ...
  return <main className={record?'recording':'shell'}>
    <section className="stage">
      <div className="phone">
        ...
        <div className="head"><i>?</i><div><b>{story.contactName}</b><small>{typing?'typing…':story.subtitle}</small></div></div>
        <div className="chat">
          <em>{story.title}</em>
          {story.events.slice(0,v).map(e=>(
            <div key={e.id}>
              {e.dividerLabel&&<div className="divider">{e.dividerLabel}<small>{e.storyTime}</small></div>}
              <div className={e.sender===story.contactName?'row in':'row out'}>
                <article>{e.type==='image'&&<img src={e.imageUrl} alt="AI story reveal"/>}<span>{e.text}</span></article>
              </div>
            </div>
          ))}
