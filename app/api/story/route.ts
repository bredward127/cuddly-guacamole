...
type Character={id:string;name:string;role:string;relationship:string;personality:string;secret:string;isMainCharacter:boolean};
...
function parseCharacters(raw:unknown):Character[]{
  ...
  return raw.slice(0,6).map((c,i)=>{
    const row=c as Partial<Character>;
    return {
      id:String(row?.id||`c${i}`),
      name:String(row?.name||'').trim().slice(0,40),
      role:String(row?.role||'').trim().slice(0,80),
      relationship:String(row?.relationship||'').trim().slice(0,120),
      personality:String(row?.personality||'').trim().slice(0,180),
      secret:String(row?.secret||'').trim().slice(0,180),
      isMainCharacter:i===0,
    };
  }).filter(c=>c.name);
}

function matchName(raw:string, names:string[]){
  const needle=raw.trim().toLowerCase();
  return names.find(name=>name.toLowerCase()===needle) || names.find(name=>needle.includes(name.toLowerCase()) || name.toLowerCase().includes(needle));
}

function applyCharacterIdentity(story:Story, characters:Character[], isGroup:boolean, groupName:string){
  const names=characters.map(c=>c.name);
  const you=characters.find(c=>c.isMainCharacter)||characters[0];
  const others=characters.filter(c=>c.id!==you.id && c.name!==you.name);
  let unknown=0;
  const events=story.events.map(e=>{
    const hit=matchName(e.sender, names);
    const sender=hit || (e.sender && others.length ? others[unknown++ % others.length].name : you.name);
    return {...e, sender, isMe:sender===you.name};
  });
  return {
    ...story,
    contactName:isGroup?(groupName||story.title):(others[0]?.name||story.contactName),
    events,
    senders:names,
    youSender:you.name,
    isGroup,
...
export async function POST(req:Request){
  try{
    ...
    const isGroup=/group|family/i.test(format);
    const groupName=String(b.groupName||'').trim().slice(0,42);
    const characters=parseCharacters(b.characters);
    if(isGroup && characters.length<3)return NextResponse.json({error:'Group chats need at least 3 named characters.'},{status:400});
    if(!isGroup && characters.length && characters.length!==2)return NextResponse.json({error:'One-on-one chats need exactly two named characters.'},{status:400});
    if(characters.length){
      const names=characters.map(c=>c.name.toLowerCase());
      if(new Set(names).size!==names.length)return NextResponse.json({error:'Character names must be unique.'},{status:400});
    }
    ...
    let i=0;
    const withImages={...story,events:story.events.map((e,idx)=>({...e,id:String(idx),imageUrl:e.type==='image'?imageUrls[i++]:''}))};
    if(characters.length){
      return NextResponse.json(applyCharacterIdentity(withImages as Story, characters, isGroup, groupName));
    }
    const senderOrder:string[]=[];
    for(const e of withImages.events){
      if(e.sender&&!senderOrder.includes(e.sender))senderOrder.push(e.sender);
    }
    const youSender=senderOrder[0]||'You';
    return NextResponse.json({...withImages,events:withImages.events.map(e=>({...e,isMe:e.sender===youSender})),senders:senderOrder,youSender,isGroup,participants:[],groupName:isGroup?groupName:''});
  }catch(e){
    console.error(e);
